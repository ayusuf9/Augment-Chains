...
def process_pdf(pdf_path):
    ...
    if os.path.exists(f"{faiss_index_file}.faiss"):
        vector_store = FAISS.load_local(faiss_index_file, embeddings=get_embedding_model())
...
def main():
    ...
    if selected_pdf:
        ...
        # Only process the PDF if it's different from the current one
        if pdf_path != st.session_state.current_pdf:
