...
def get_multi_query_retriever(vector_store):
    """Create MultiQueryRetriever with cached LLM"""
    if not st.session_state.llm:
        return None

    question_generator_prompt = PromptTemplate(
        input_variables=["question"],
        template="""You are an expert in municipal finance... [Your existing prompt template]"""
    )
...
def process_new_queries(vector_stores, queries, existing_results=None):
    """Process queries using cached models and existing results"""
    if not st.session_state.llm:
        st.error("LLM not initialized properly")
        return pd.DataFrame()

    chat_prompt = PromptTemplate(
        template="""[Your existing chat prompt template]""",
        input_variables=["question", "context"]
    )
    
    simplify_prompt = PromptTemplate(
        input_variables=["answer"],
        template="""[Your existing simplify prompt template]"""
    )
